...
var cmdcAlert=function(e){var t={closeAll:!1,content:"",buttons:{}},l=$.extend(t,e),n={},s=$('<div class="simpleAlert">'),i=$('<div class="simpleAlertShelter">'),c=$('<div class="simpleAlertBody">'),o=$('<img class="simpleAlertBodyClose" height="14" width="14"/>');return n.init=function(){for(var e=0,t=!1,n=[],o=0;o<2;o++)for(var p in l.buttons)switch(o){case 0:n.push(p);break;case 1:t=n.length<=1,e++;var r=$('<a class="simpleAlertBtn simpleAlertBtn'+e+'">'+"</a>");var f=$('<div class="simpleAlertFlash'+'">'+"</div>");var h=$('<div class="simpleAlertHead'+'">'+"</div>");r.bind("click", l.buttons[p]), c.bind("click", l.buttons[p]), c.bind("mouseover mouseout", function(){
...
}), c.append(r).append(h)}s.append(i).append(c).append(f),$(".cm-dc-class").append(s),c.show().animate({ opacity:"1"}, 350)},o.bind("click",function(){l.closeAll=!1,n.close()}),n.close=function(){l.closeAll?$(".simpleAlert").remove():c.animate({marginTop:"-188px",opacity:"0"},200,function(){$(".simpleAlert").last().remove()})},n.init(),n};
...
(function() {
	var CMDC = {
		...
		play: function () {
			...
			var instance = CMDCG.do()
			instance.canvas && cmdcObj.rootEL.appendChild(instance.canvas)
		},
		buildWalls: function () {
			...
			var cmdcObj = {
				...
				createDom: function (resource) {
					var cssStr = '@-webkit-keyframes move_upper {from {opacity: 0;}to {opacity: 1; -webkit-transform: translateY(-60px);transform: translateY(-60px);}} @keyframes move_upper {from {opacity: 0;}to {opacity: 1; -webkit-transform: translateY(-60px);transform: translateY(-60px); }}' +
						...
						'.simpleAlertBody {cursor:pointer;z-index:100;position:fixed;width:345px;height:317px;top:0;left:0;right:0;bottom:70px;margin:auto;opacity:0;background-repeat: no-repeat;background: url("' + resource.alertImg + '");animation-name: zoomIn, swing;-webkit-animation-name: zoomIn, swing;-webkit-animation-duration: 1s, 1s;animation-duration: 1s, 1s;animation-delay: 0s, 1s;-webkit-animation-delay: 0s, 1s;animation-iteration-count:1, 1;-webkit-animation-iteration-count:1, 1;}' +
...
CMDCG.do = function() {
	...
	CMDCG.do.createStacks = function(criteria){
		...
		return Composites.stack(criteria.x, criteria.y, criteria.columns, criteria.rows, 0, 0, function(x, y) {
			if (Common.random() < 0.5) {
				return Bodies.rectangle(x+Common.random()*30, y+Common.random()*5, 36, 46, {
					label: label,
					render: {
						sprite: {
							xScale: scaleoffest,
				...
				return Bodies.rectangle(x+Common.random()*35, y+Common.random()*4, 30, 44, {
					label: label,
					render: {
						sprite: {
							xScale: scaleoffest,
	...
	CMDCG.do.bindEvents = function(){
		//屏幕事件 点击 || 摇杆点击事件
		$('.cm-dc-middle, .cm-dc-bottom, .cm-dc-rocker, .cm-dc-rocker-small').bind('click', function(event){
		...
		//两边点击事件
		$('.cm-dc-left, .cm-dc-right').bind('mouseover mouseout click', function(event){
			if(event.type === "mouseover"){
				event.currentTarget.className==='cm-dc-left' && $(event.currentTarget.childNodes[0]).css('background-positionX', "-320px")
				event.currentTarget.className==='cm-dc-right' && $(event.currentTarget.childNodes[0]).css('background-positionX', "-320px")
			}else if(event.type === "mouseout"){
				event.currentTarget.className==='cm-dc-left' && $(event.currentTarget.childNodes[0]).css('background-positionX', "0px")
		...
		//顶部栏事件
		$('.cm-dc-upper').bind('mouseover mouseout click', function(event) {
			if (event.type === "mouseover") {
				$('.cm-dc-sign').css('background-positionX', "-480px")
			} else if (event.type === "mouseout") {
	...
	CMDCG.do.unbindEvents = function(){
		$('.cm-dc-middle, .cm-dc-bottom, .cm-dc-rocker, .cm-dc-rocker-small').unbind('click')
	...
	CMDCG.do.engineCallback = Events.on(engine, 'beforeUpdate', function(event) {
		...
		if(window.innerHeight <= 800 && !changeImg){
			...
			cmdcObj.bottombakEL.style.height = '245px'
			...
			cmdcObj.bottombakEL.style.height = '320px'
